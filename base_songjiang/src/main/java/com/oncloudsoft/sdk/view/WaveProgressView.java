...
import com.oncloudsoft.sdk.utils.SizeUtil;
...
public class WaveProgressView extends View {
    ...
    private int WaveTheme = 0;//默认浅绿色 浅绿色0       当值在25到60之间是 深绿色 1深绿色     当值大于60 之后为红色 2红色
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        waveWidth = typedArray.getDimension(R.styleable.WaveProgressView_wave_width, SizeUtil.dip2px(context, 25));
        waveHeight = typedArray.getDimension(R.styleable.WaveProgressView_wave_height, SizeUtil.dip2px(context, 5));
        ...
        defaultSize = SizeUtil.dip2px(context, 100);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int corcleFrameColor = R.color.lyrical_blue_01;
        int fristWaveColor = R.color.lyrical_blue_02;
        int secondWaveColor = R.color.lyrical_blue_03;
        int backgroundWaveColor = R.color.lyrical_blue_04;


        switch (WaveTheme) {
            case 0://浅绿色
                corcleFrameColor = R.color.lyrical_blue_01;
                fristWaveColor = R.color.lyrical_blue_02;
                secondWaveColor = R.color.lyrical_blue_03;
                backgroundWaveColor = R.color.lyrical_blue_04;
                break;
            case 1://深绿色
                corcleFrameColor = R.color.lyrical_green_01;
                fristWaveColor = R.color.lyrical_green_02;
                secondWaveColor = R.color.lyrical_green_03;
                backgroundWaveColor = R.color.lyrical_green_04;
                break;
            case 2://红色
                corcleFrameColor = R.color.lyrical_red_01;
                fristWaveColor = R.color.lyrical_red_02;
                secondWaveColor = R.color.lyrical_red_03;
                backgroundWaveColor = R.color.lyrical_red_04;
                break;
        }


        circlePaint.setColor(ContextCompat.getColor(getContext(), corcleFrameColor));
        circlePaint.setStrokeWidth(3); //线宽
        circlePaint.setStyle(Paint.Style.STROKE);//首先画边框
        bitmapCanvas.drawCircle(viewSize / 2, viewSize / 2, viewSize / 2 - 2, circlePaint);

        circlePaint.setColor(ContextCompat.getColor(getContext(), backgroundWaveColor));
        circlePaint.setStyle(Paint.Style.FILL);//画内容
        bitmapCanvas.drawCircle(viewSize / 2, viewSize / 2, viewSize / 2 - 2, circlePaint);

        wavePaint.setColor(ContextCompat.getColor(getContext(), fristWaveColor));
        bitmapCanvas.drawPath(getWavePath(), wavePaint);
        if (isDrawSecondWave) {
            secondWavePaint.setColor(ContextCompat.getColor(getContext(), secondWaveColor));
...
//        float changeWaveHeight = (1 - percent) * waveHeight;
     ...
     * @param time        动画持续时间
     */
    public void setProgressNum(float progressNum, int time) {//默认浅绿色 浅绿色0       当值在25到60之间是 深绿色 1深绿色  当值大于60 之后为红色 2红色

        if (progressNum < 0) {
            progressNum = 0;
        } else if (progressNum < 25) {//当值在25到60之间是 浅绿色 1浅绿色
            WaveTheme = 0;
        } else if (progressNum < 61) {
            WaveTheme = 1;
        } else {
            WaveTheme = 2;
        }
