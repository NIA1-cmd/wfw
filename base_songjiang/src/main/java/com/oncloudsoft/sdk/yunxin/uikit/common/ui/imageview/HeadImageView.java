package com.oncloudsoft.sdk.yunxin.uikit.common.ui.imageview;
...
public class HeadImageView extends AppCompatImageView {
    ...
    public void loadBuddyAvatar(String account) {
        ...
        int header = DEFAULT_AVATAR_RES_ID;
        if (account.contains("fg")) {
            header = R.drawable.icon_team_fg;
        } else if (account.contains("sys")) {
            header = R.drawable.head_sys2;
        } else if (MyApplication.getTeamExpansionData().getSqrid().contains(account)) {//申请人
            header = R.drawable.icon_team_sqr;

        } else if (MyApplication.getTeamExpansionData().getBzxrid().contains(account)) {
            header = R.drawable.icon_team_bzxr;
        }

        doLoadImage(null, header, DEFAULT_AVATAR_THUMB_SIZE);
    }
