package com.oncloudsoft.sdk.yunxin.uikit.business.ait.selector;
...
import com.oncloudsoft.sdk.yunxin.uikit.business.ait.selector.adapter.AitContactAdapter;
import com.oncloudsoft.sdk.yunxin.uikit.business.ait.selector.model.AitContactItem;
import com.oncloudsoft.sdk.yunxin.uikit.business.ait.selector.model.ItemType;
...
public class AitContactSelectorActivity extends UI {
    ...
    private void initViews() {
        RecyclerView recyclerView = findViewById(R.id.member_list);
         titleView = findViewById(R.id.bt_title);

        titleView.setTitleText("选择提醒的人");
        titleView.setOnLeftTextClickListener(new BaseTitleView.onLeftTextClickListener() {
            @Override
            public void onClick() {
                finish();
            }
...
//                        items.add(new AitContactItem(ItemType.SIMPLE_LABEL, "群成员"));
                        for (TeamMember member : members) {
                            ...
                            AitContactItem aitContactItem = new AitContactItem(ItemType.TEAM_MEMBER, member);
                            TeamMember model = (TeamMember) aitContactItem.getModel();

                            String account = model.getAccount();

                            if (account != null && !account.contains("sys")) {//不需要显示系统
                                items.add(aitContactItem);
