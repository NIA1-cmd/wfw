...
public abstract class MsgViewHolderBase extends RecyclerViewHolder<BaseMultiItemFetchLoadAdapter, BaseViewHolder, IMMessage> {
    ...
    // view
    View alertButton;
    protected RelativeLayout mRlParentBaseItemLayout;//每条消息的父布局
    ...
    public void convert(BaseViewHolder holder, IMMessage data, int position, boolean isScrolling) {
        ...
        inflate(message.getFromAccount());
        refresh();
        bindHolder(holder);
    }

    protected final void inflate(String account) {
        mRlParentBaseItemLayout = findViewById(R.id.rl_baseitem_parent);
        ...
        //messageCreate = findViewById(R.id.message_createtime);
        ...
        //        avatarRight.setImageBitmap(BitmapFactory.decodeResource(context.getResources(),R.drawable.head_judge2));
    ...
    private void setTimeTextView() {
        if (getMsgAdapter().ShowTimeItem() == true) {
            if (getMsgAdapter().needShowTime(message.getTime()+"")) {//因为查询聊天记录没有uuid字段 所以用时间代替
            ...
            if (getMsgAdapter().needShowTime(message.getUuid())) {
                timeTextView.setVisibility(View.VISIBLE);
            }
            else {
                timeTextView.setVisibility(View.GONE);
    ...
    /**
     * 从消息搜索界面进入 会返回true 显示发送时间
     */
    /*private void setSendTime() {
        if (getMsgAdapter().ShowTimeItem()==true) {
            if (isReceivedMessage()){
                messageCreate.setGravity(View.FOCUS_LEFT);
                messageCreate.setPadding(0,0,50,0);
            }else{
                messageCreate.setGravity(View.FOCUS_RIGHT);
                messageCreate.setPadding(50,0,0,0);

            }
            messageCreate.setVisibility(View.VISIBLE);

            //间隔超过五分钟才显示发送时间
            long time1 = message.getTime();
            if (time1-time>(NimUIKitImpl.getOptions().displayMsgTimeWithInterval)){
                String text = TimeUtil.getTimeShowString(message.getTime(), false);
                messageCreate.setText(text);
                messageCreate.setVisibility(View.VISIBLE);
            }else{
                messageCreate.setVisibility(View.GONE);
            }
            time = time1;
    ...
    protected String getNameText() {
        if (message.getSessionType() == SessionTypeEnum.Team) {
            // TODO: 2019/3/25   昵称后面追加该角色在该群中的角色
    ...
    public void setTextColor(TextView textColor, @ColorRes int color) {
        textColor.setTextColor(context.getResources().getColor(color));
