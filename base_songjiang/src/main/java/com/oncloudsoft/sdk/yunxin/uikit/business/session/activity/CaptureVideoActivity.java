...
import com.oncloudsoft.sdk.yunxin.uikit.common.ui.dialog.EasyAlertDialogHelper;
...
public class CaptureVideoActivity extends UI implements SurfaceHolder.Callback {
    ...
    private static final int VIDEO_TIMES = 3 * 60 * 60;//限制视屏时长为3小时
    ...
    private int flag;//1:群聊中发送的视屏     2  ：执行记录中发送的视屏
    ...
    private static onHandlerVideoCallBack onHandlerVideoCallBack;


    public interface onHandlerVideoCallBack {
        void onBack();
    ...
    public static void startFromForensics(Activity activity, String videoFilePath, int captureCode, int mFlag,String type, onHandlerVideoCallBack callBack) {
        onHandlerVideoCallBack = callBack;
        Intent intent = new Intent();
        intent.setClass(activity, CaptureVideoActivity.class);
        intent.putExtra(EXTRA_DATA_FILE_NAME, videoFilePath);
        intent.putExtra("from", mFlag);
        activity.startActivityForResult(intent, captureCode);
    }
    ...
    private void parseIntent() {
        ...
        flag = getIntent().getExtras().getInt("from");
    }
    ...
    private void startRecorder() {
        ...
        if (flag == 2) {//说明是执行记录中
            onHandlerVideoCallBack.onStart();
...
//    public void creatRecord(String caseId, int stage) {
//
//        HttpParams params = HttpParams.HttpParams()
//                .add("ajbs", caseId);
//
//
//        if (!String.valueOf(stage).equals("7")) {//选择的是不关联节点
//            params.add("jdbs", String.valueOf(stage));
//        }
//        Location location = LocationUtils.getInstance(CaptureVideoActivity.this).showLocation();
//        if (location != null) {
//            params.add("lon", location.getLongitude() + "");//经度
//            params.add("lat", location.getLatitude() + "");//纬度
//        }
//        params.add("wjlx", "2");//视屏
//        params.add("qzsj", DateUtil.getCurrentTiem());
//
//        MyOkhttpClient.getOkhttpInstance().sentGet(CaptureVideoActivity.this, Global.URL_CREATERECORD, params.build(), new MyOkhttpClient.MyOkhttpCallBack() {
//            @Override
//            public void onRequestSccess(String json) throws JSONException {
//                recordId = json;
//
//                MyLog.d("创建取证记录成功" + json);
//
//            }
//
//            @Override
